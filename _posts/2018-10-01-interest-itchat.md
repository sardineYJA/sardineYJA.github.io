...
无意中看到itchat库，发现还是挺有趣的，可以自定义实现微信的功能
...
```python
# hotReload=true 表示保持登录，生成文件itcht.pkl存储登录状态
# loginCallback 登录完成后的方法
# exitCallback 退出后的方法
# enableCmdQR=True  命令行显示二维码
...
res = itchat.search_friends('天降测试')  # 备注名或昵称
print(res)
UserName = res[0]['UserName']  # 发送信息所需的toUserName
print(UserName)
itchat.send('Hello, Test', toUserName=UserName)   # 发送信息
```

## 发送消息，文件

```python
itchat.send("Hello World!")   # 发送信息，toUserName:发送对象, 如果留空将发送给自己
itchat.send_msg("Hello World!")
ithcat.send("@fil@%s" % '/tmp/test.text')   # 发送文件
ithcat.send("@img@%s" % '/tmp/test.png')    # 发送图片
ithcat.send("@vid@%s" % '/tmp/test.mkv')    # 发送视频
```
